...
namespace GalaxySMS.GalaxyHardware.ViewModels
{
    ...
    public class EditAreaViewModel : ViewModelBase, ISupportsUserEntitySelection
    {
        ...
        public EditAreaViewModel(IEventAggregator eventAggregator, IClientServices clientServices, Entities.Area entity, Guid instanceId)
        {
            ...
            _Area = new Entities.Area(entity);
            _Area.CleanAll();
            ...
            CurrentItemTitle = _Area.Display;
            ...
            if (UserSessionToken != null)
            {
                ...
                {
                    if (_Area.EntityIds.Contains(ue.EntityId))
                        ue.Selected = true;
                }
            }
        }
        ...
        private async void OnSaveCommandExecute(object arg)
        {
            ...
            if (IsValid)
            {
                BusyContent = string.Format(CommonResources.Resources.EditAreaView_PleaseWaitWhileISave, Area.Display);

                Area.EntityIds.Clear();
                foreach (var ue in UserEntitiesSelectionList)
                {
                    if (ue.Selected || ue.EntityId == _clientServices.CurrentSite.EntityId)
                        Area.EntityIds.Add(ue.EntityId);
                }

                IsBusy = true;
                var manager = _clientServices.GetManager<GalaxyAreaManager>();
                bool isNew = (Area.AreaUid == Guid.Empty);
                Entities.Area savedEntity;
                var parameters = new SaveParameters<Entities.Area>()
                {
                    Data = Area,
                ...
                {
                    savedEntity = manager.SaveArea(parameters);
                }
                else
                {
                    savedEntity = await manager.SaveAreaAsync(parameters);
                }

                if (savedEntity != null && manager.HasErrors == false)
                {
                    _eventAggregator.GetEvent<PubSubEvent<EntitySavedEventArgs<Entities.Area>>>().Publish(new EntitySavedEventArgs<Entities.Area>()
                    {
                        Entity = savedEntity,
                        IsNew = isNew
                    });
        ...
        {
            return _Area.IsDirty;// || _Cluster.Address.IsDirty;
        }

        private void OnCancelCommandExecute(object arg)
        {
            _eventAggregator.GetEvent<PubSubEvent<OperationCanceledEventArgs<Entities.Area>>>().Publish(new OperationCanceledEventArgs<Entities.Area>()
            {
                Entity = Area,
