...
namespace GalaxySMS.Client.SDK.Managers
{
    ...
    public class ClusterManager : ManagerBase
    {
            ...
            WithClient<ISystemManagementService>(
                _ServiceFactory.CreateClient<ISystemManagementService>(Binding,
                    GetServiceAddress(ServiceNames.SystemManagementService), ClientUserSessionData), proxy =>
                    {
                        try
                        {
                            data = proxy.GetClustersWithGalaxyPanelInfo(parameters);
                        }
                        catch (FaultException<ExceptionDetail> ex)
                        {
                            AddError(new CustomError(ex.Detail));
        ...
        public async Task<ArrayResponse<ClusterGalaxyPanelMinimal>> GetClustersWithGalaxyPanelInfoAsync(GetParametersWithPhoto parameters)
        {
            InitializeErrorsCollection();
            var data = new ArrayResponse<ClusterGalaxyPanelMinimal>();
            try
            {
                return await WithClientFuncAsync(
                    _ServiceFactory.CreateClient<ISystemManagementService>(Binding,
                        GetServiceAddress(ServiceNames.SystemManagementService), ClientUserSessionData), async proxy =>
                        {
                            data = await proxy.GetClustersWithGalaxyPanelInfoAsync(parameters);
                            return data;
                        });
            }
            catch (FaultException<ExceptionDetail> ex)
