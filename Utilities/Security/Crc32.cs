...
namespace GCS.Security
{
    public class Crc32
    {
        ...
        static public uint UPDC32(byte octet, uint crc)
        {
            return (crc_32_tab[((crc) ^ ((byte)octet)) & 0xff] ^ ((crc) >> 8));
        }

        static public uint UPDC32Test(byte octet, uint crc)
        {
            return crc_32_tab[octet] ^ (crc >> 8);
        }
        ...
        //static public uint CRC32Bytes(byte[] bytes, int len)
        //{
        //    uint oldcrc32;
        //    oldcrc32 = 0xFFFFFFFF;
        ...
        //static public uint CRC32BytesReverse(byte[] bytes, int len)
        //{
        //    uint oldcrc32;
        //    oldcrc32 = 0xFFFFFFFF;
        ...
        static public uint GetCRC(byte[] buffer)
        {                                               // create the check sequence to verify the message
            byte c, i;                             // comes from the real DLL
            uint code = 0;
            ...
            while (length-- > 0)
            {
                ...
                {
                    if (((code ^ c) & 3) != 0)
                        code = (code >> 1) ^ 0x85014521;
                    else if (((code ^ c) & 1) != 0)
                        code = (code >> 1) ^ 0x80001080;
                    else
                        code = (code >> 1) ^ 0x40011040;
                }
            }
            return code;
