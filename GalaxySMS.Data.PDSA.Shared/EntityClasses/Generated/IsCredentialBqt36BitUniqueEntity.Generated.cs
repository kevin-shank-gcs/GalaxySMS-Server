using System;
using System.ComponentModel;
using System.Runtime.Serialization;

using PDSA.Validation;

namespace GalaxySMS.EntityLayer
{
  /// <summary>
  /// This class contains properties for each data value returned, or parameter to be input/output from the IsCredentialBqt36BitUniquePDSA stored procedure.
  /// This class is generated by the Haystack Code Generator for .NET.
  /// Do not modify this class or add methods as it is intended to be able to be re-generated at any time.
  /// </summary>
  
  public partial class IsCredentialBqt36BitUniquePDSA : PDSAEntityBase
  {
    #region Private Variables
    private int _Result = 0;
    private Guid _CredentialUid = Guid.Empty;
    private short _FacilityCode = 0;
    private int _IdCode = 0;
    private short _IssueCode = 0;
    private int _RETURNVALUE = 0;
    #endregion
    
    #region Public Properties
    /// <summary>
    /// Get/Set the Result value
    /// </summary>
    
    [DataMember]
    
    public int Result 
    { 
      get { return _Result; }
      set 
      { 
        if(HasValueChanged(_Result, value, "Result"))
        {
          _Result = value; 
          RaisePropertyChanged("Result");
        }
      } 
    }
        
    /// <summary>
    /// Get/Set the Credential Uid value
    /// </summary>
    
    [DataMember]
    
    public Guid CredentialUid 
    { 
      get { return _CredentialUid; }
      set 
      { 
        if(HasValueChanged(_CredentialUid, value, "@CredentialUid"))
        {
          _CredentialUid = value; 
          RaisePropertyChanged("CredentialUid");
        }
      } 
    }
        
    /// <summary>
    /// Get/Set the Facility Code value
    /// </summary>
    
    [DataMember]
    
    public short FacilityCode 
    { 
      get { return _FacilityCode; }
      set 
      { 
        if(HasValueChanged(_FacilityCode, value, "@FacilityCode"))
        {
          _FacilityCode = value; 
          RaisePropertyChanged("FacilityCode");
        }
      } 
    }
        
    /// <summary>
    /// Get/Set the Id Code value
    /// </summary>
    
    [DataMember]
    
    public int IdCode 
    { 
      get { return _IdCode; }
      set 
      { 
        if(HasValueChanged(_IdCode, value, "@IdCode"))
        {
          _IdCode = value; 
          RaisePropertyChanged("IdCode");
        }
      } 
    }
        
    /// <summary>
    /// Get/Set the Issue Code value
    /// </summary>
    
    [DataMember]
    
    public short IssueCode 
    { 
      get { return _IssueCode; }
      set 
      { 
        if(HasValueChanged(_IssueCode, value, "@IssueCode"))
        {
          _IssueCode = value; 
          RaisePropertyChanged("IssueCode");
        }
      } 
    }
        
    /// <summary>
    /// Get/Set the return value value
    /// </summary>
    
    [DataMember]
    
    public int RETURNVALUE 
    { 
      get { return _RETURNVALUE; }
      set 
      { 
        if(HasValueChanged(_RETURNVALUE, value, "@RETURN_VALUE"))
        {
          _RETURNVALUE = value; 
          RaisePropertyChanged("RETURNVALUE");
        }
      } 
    }
        

    #endregion
  }
}
