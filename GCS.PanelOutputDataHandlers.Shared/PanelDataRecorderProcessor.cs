...
{
    public class PanelDataRecorderProcessor : IPanelOutputDataProcessor
    {
        private CancellationTokenSource _cancellationTokenSource;
        private CancellationToken _cancellationToken;
        public SystemType SystemType {get; private set;}

        public PanelDataRecorderProcessor()
        {
        ...
        public void ProcessData(System.Collections.Concurrent.BlockingCollection<object> queue)
        {
            ...
            Task task = new Task(() =>
            {
                ...
                this.Log().DebugFormat("*** {0}: PanelDataRecorderProcessor Task starting", DateTimeOffset.Now.ToString());
                IPanelOutputDataMessageHandler recorder = new PanelPacketDataRecorder();
...
//                        GCS.Framework.Logging.LogWriter.LogDebugInformation("PanelOutputDataProcessor cancellation requested");
                        this.Log().DebugFormat("PanelDataRecorderProcessor cancellation requested");
                ...
                this.Log().DebugFormat("*** {0}: PanelDataRecorderProcessor Task ending", DateTimeOffset.Now.ToString());
