...
namespace GalaxySMS.Api.Controllers.Api
{
   ...
   // [ApiExplorerSettings(IgnoreApi = true)]
    ...
    public class MercScpController : ControllerBaseEx
    {
        ...
        ///
        /// <returns>   A Task&lt;ActionResult&lt;ApiEntities.MercScpBasic[]&gt;&gt; </returns>
        ///=================================================================================================
        ...
        //[MapToApiVersion("1.0")]
        public async Task<ActionResult<ApiEntities.ArrayResponse<ApiEntities.MercScpBasic>>> Get(int pageSize, int pageNumber, bool includeChildren = false, bool includeCommands = false, MercScpSortProperty sortBy = MercScpSortProperty.ScpName, OrderBy orderBy = OrderBy.Asc)
        {
            try
            {
                ...
                var resp = Mapper.Map<ApiEntities.ArrayResponse<ApiEntities.MercScpBasic>>(results);
                return Ok(resp);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiEntities.ArrayResponse<ApiEntities.MercScpBasic>>> GetGalaxyControllersForSite(Guid siteUid, bool includeChildren = false, bool includeCommands = false)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                var mgr = Helpers.GetManager<MercScpManager>(ServerWcfServerAddress, ServerWcfBindingType, ServerUserName, ServerPassword, ClientUserSessionData);
                var results = await mgr.GetMercScpsForSiteAsync(new WcfEntities.GetParametersWithPhoto()
                {
                    UniqueId = siteUid,
                    IncludePhoto = false,
                    IncludeMemberCollections = includeChildren,
                ...
                var resp = new ApiEntities.ArrayResponse<ApiEntities.MercScpBasic>();
                resp.Items = Mapper.Map<ApiEntities.MercScpBasic[]>(results);
                return Ok(resp);
            }
            catch (Exception ex)
            {
        ...
        ///=================================================================================================
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [HttpGet("allforscpgroup/{scpGroupUid}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        ...
        public async Task<ActionResult<ApiEntities.ArrayResponse<ApiEntities.MercScpBasic>>> GetMercScpsForMercScpGroup(Guid scpGroupUid, bool includeChildren = false, bool includeCommands = false)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                var mgr = Helpers.GetManager<MercScpManager>(ServerWcfServerAddress, ServerWcfBindingType, ServerUserName, ServerPassword, ClientUserSessionData);
                var results = await mgr.GetMercScpsForMercScpGroupAsync(new WcfEntities.GetParametersWithPhoto()
                {
                    UniqueId = scpGroupUid,
                    IncludePhoto = false,
                    IncludeMemberCollections = includeChildren,
                    IncludeCommands = includeCommands
                });
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>   Get a panel by MercScpUid value. </summary>
        /// 
        /// <param name="mercScpUid">   The panel UID. </param>
        /// <param name="includeChildren">  (Optional) True to include, false to exclude the children. </param>
        ///
        /// <returns>   A Task&lt;ActionResult&lt;ApiEntities.MercScp&gt;&gt; </returns>
        ///=================================================================================================
        ...
        // If no MapToApiVersion attribute is specified, then this endpoint will work for all versions
        //[MapToApiVersion("1.0")]
        public async Task<ActionResult<ResponseModels.MercScpResp>> Get(Guid mercScpUid, bool includeChildren = false)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                //wcfGetParams.ExcludeMemberCollectionSettings.Add(new KeyValuePair<string, bool>(nameof(WcfEntities.MercScpGroup.MercScps), true));
                //wcfGetParams.ExcludeMemberCollectionSettings.Add(nameof(WcfEntities.MercScpGroup.MercScps));
                var result = await mgr.GetMercScpAsync(wcfGetParams);
                ...
                if (result == null)
                    return NotFound();
                var model = Mapper.Map<ResponseModels.MercScpResp>(result);
                return Ok(model);
            }
        ...
        public async Task<ActionResult<ApiEntities.MercScpEditingData>> GetEditorData()
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                var mgr = Helpers.GetManager<MercScpManager>(ServerWcfServerAddress, ServerWcfBindingType, ServerUserName, ServerPassword, ClientUserSessionData);
                var result = await mgr.GetMercScpEditingDataAsync(new WcfEntities.GetParametersWithPhoto()
                {
                });
                ...
                if (result == null)
                    return NotFound();
                var model = Mapper.Map<ApiEntities.MercScpEditingData>(result);
                return Ok(model);
            }
            catch (FaultException<NetCoreCommon.GCS.Core.Common.ServiceModel.ExceptionDetailEx> ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<ApiEntities.MercScp>> Post(RequestModels.SaveParams<RequestModels.MercScpReq> parameters)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                if (parameters.Data.MercScpGroupUid == Guid.Empty)
                    return this.StatusCode(StatusCodes.Status403Forbidden, $"Cannot add MercScp with MercScpGroupUid value of {parameters.Data.MercScpGroupUid}");
                ...
                var wcfEntity = Mapper.Map<WcfEntities.MercScp>(parameters.Data);
                wcfEntity.ConcurrencyValue = 0;

                if (wcfEntity.MercScpTypeUid == Guid.Empty)
                {
                    switch (parameters.Data.MercScpType)
                    {
                        //case Common.Enums.MercScpModel.MercScp5xx:
                        //    if (wcfEntity.PanelNumber > 254)
                        //        return BadRequest($"Cannot add MercScp with PanelNumber > 254 for model {parameters.Data.MercScpModel}");
                        //    wcfEntity.MercScpModelUid = GalaxySMS.Common.Constants.MercScpTypeIds.MercScpType_500;
                        //    cpuTypeUid = GalaxySMS.Common.Constants.GalaxyCpuTypeIds.GalaxyCpuType_500;
                        //    break;
                        //case Common.Enums.MercScpModel.MercScp600:
                        //    if (wcfEntity.PanelNumber > 254)
                        //        return BadRequest($"Cannot add MercScp with PanelNumber > 254 for model {parameters.Data.MercScpModel}");

                        //    wcfEntity.MercScpModelUid = GalaxySMS.Common.Constants.MercScpTypeIds.MercScpType_600;
                        ...
                        case MercuryPanelType.LP1502:
                            wcfEntity.MercScpTypeUid = GalaxySMS.Common.Constants.MercScpTypeIds.Lp1502;
                            break;

                        case MercuryPanelType.LP2500:
                            wcfEntity.MercScpTypeUid = GalaxySMS.Common.Constants.MercScpTypeIds.Lp2500;
                            break;
                        ...
                        case Common.Enums.MercuryPanelType.LP1501:
                        default:
                            wcfEntity.MercScpTypeUid = GalaxySMS.Common.Constants.MercScpTypeIds.Lp1501;
                            break;
                    }
                }
                ...
                var wcfParams = new WcfEntities.SaveParameters<WcfEntities.MercScp>(wcfEntity, wcfSaveParameters);

                var results = await mgr.SaveMercScpAsync(wcfParams);
                if (results != null)
                {
                    var savedItem = Mapper.Map<ApiEntities.MercScp>(results);
                    var url = LinkGenerator.GetPathByAction("Get",
                        "MercScp",
                        new { mercScpUid = savedItem.MercScpUid });
        ...
        public async Task<ActionResult<ApiEntities.MercScp>> Put(RequestModels.SaveParams<RequestModels.MercScpPutReq> parameters)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                if (parameters.Data.MercScpGroupUid == Guid.Empty)
                    return this.StatusCode(StatusCodes.Status403Forbidden, $"Cannot update MercScp with MercScpGroupUid value of {parameters.Data.MercScpGroupUid}");
                //if (parameters.Data.MercScpUid == Guid.Empty)
                //    return this.StatusCode(StatusCodes.Status403Forbidden, $"Cannot update MercScp with MercScpUid value of {parameters.Data.MercScpUid}");
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>
        /// Delete a specific panel by mercScpUid.
        /// </summary>
        ///
        /// <param name="mercScpUid"> Identifier for the Merc Panel to delete. </param>
        ///
        /// <returns>   A Task&lt;IActionResult&gt; </returns>
        ///=================================================================================================        
        [HttpDelete("{mercScpUid}")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Delete(Guid mercScpUid)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
                ...
                var mgr = Helpers.GetManager<MercScpManager>(ServerWcfServerAddress, ServerWcfBindingType, ServerUserName, ServerPassword, ClientUserSessionData);
                var result = await mgr.GetMercScpAsync(new WcfEntities.GetParametersWithPhoto()
                {
                    UniqueId = mercScpUid,
                    IncludePhoto = false,
                ...
                var x = await mgr.DeleteMercScpByUniqueIdAsync(new WcfEntities.DeleteParameters() { UniqueId = mercScpUid });
                if (mgr.HasErrors)
                {
                    return GetStatusCodeResult(mgr);
                }
        ...
        //public async Task<IActionResult> ExecuteCommand(RequestModels.CommandParams<RequestModels.MercScpGroupCommandActionReq> parameters)
        //{
        //    try
        //    {
        //        var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
        ...
        //        var mgr = Helpers.GetManager<MercScpManager>(ServerWcfServerAddress, ServerWcfBindingType, ServerUserName, ServerPassword, ClientUserSessionData);
        //        var wcfParams = Mapper.Map<WcfEntities.CommandParameters<WcfEntities.MercScpGroupCommandAction>>(parameters);
        ...
        // If no MapToApiVersion attribute is specified, then this endpoint will work for all versions
        //[MapToApiVersion("1.0")]
        public async Task<ActionResult<ApiEntities.ArrayResponse<ApiEntities.ActivityHistoryEvent>>> GetActivityHistory([FromQuery] RequestModels.MercScpActivityHistoryEventSearchParametersReq parameters)
        {
            try
            {
                var sessionId = this.GetClaimGuid(GalaxySMSClaimTypes.SessionId.ToString());
