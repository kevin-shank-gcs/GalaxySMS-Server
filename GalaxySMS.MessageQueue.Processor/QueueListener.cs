...
using GalaxySMS.MessageQueue.Processor.Properties;
...
namespace GalaxySMS.MessageQueue.Processor
{
    public class QueueListener
    {
        ...
        public async void Start(string queueName)
        {
            ...
            ushort signalRPort = 0;
            bool bUseSignalR = true;
            var value = ConfigurationManager.AppSettings["signalRPort"];
            if (value != null)
            {
                if (ushort.TryParse(value.ToString(), out signalRPort))
                {
            ...
            var useSignalR = ConfigurationManager.AppSettings["useSignalR"];
            if (useSignalR != null)
            {
                if (bool.TryParse(useSignalR.ToString(), out bUseSignalR))
                {
            ...
            if (signalRPort > 0 && bUseSignalR)
            {
                ...
                _signalRClient.ConnectionClosedEvent += _signalRClient_ConnectionClosedEvent;
                _signalRUrl = string.Format("http://localhost:{0}/signalr", signalRPort);
                _signalRClientConnected = await ConnectToSignalR();
#endif
            }
            else
            ...
            switch (queueName)
            {
                ...
                case QueueNames.PanelMessagePublisher:
                    //                    StartListening(queueName, MessagePattern.FireAndForget);
           ...
           where TWorkflow : IPanelMessageWorkflow, new()
        {
            try
            {
                ...
                queue.Listen(x =>
                {
                    ...
                    if (data != null)
                    {
                        if (_signalRClientConnected == false && _signalRPort > 0)
                        {
#if NETFRAMEWORK
                            Task.Run(async () =>
                            {
