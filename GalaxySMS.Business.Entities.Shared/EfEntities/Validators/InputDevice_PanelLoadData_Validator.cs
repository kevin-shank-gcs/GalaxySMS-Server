...
#endif
{
    ...
    {
        class InputDevice_PanelLoadDataValidator : AbstractValidator<InputDevice_PanelLoadData>
        {
            public InputDevice_PanelLoadDataValidator()
            {
                //RuleFor(obj => obj.InputName).NotEmpty().Length(1, 65);
                //RuleFor(obj => obj.ClusterGroupId).Length(0, 16);
                RuleFor(obj => (int)obj.ModuleNumber).InclusiveBetween(GalaxyRemoteInputModuleLimits.LowestDefinableModuleNumber, GalaxyRemoteInputModuleLimits.HighestDefinableModuleNumber);
                RuleFor(obj => (int)obj.NodeNumber).InclusiveBetween(GalaxyRemoteInputModuleLimits.LowestDefinableInputNumber, GalaxyRemoteInputModuleLimits.HighestDefinableInputNumber);
                RuleFor(obj => (int)obj.DelayDuration).InclusiveBetween(0, ushort.MaxValue);
                RuleFor(obj => (int)obj.ArmControlScheduleNumber).InclusiveBetween(TimeScheduleLimits.Never, TimeScheduleLimits.Always);
        ...
        {
            return new InputDevice_PanelLoadDataValidator();
        }
    }
}
