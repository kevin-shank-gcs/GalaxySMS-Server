...
namespace GalaxySMS.Client.SDK.Managers
{
    ...
    public class UserManager : ManagerBase
    {
                ...
                _ServiceFactory.CreateClient<IAdministrationService>(Binding,
                    GetServiceAddress(ServiceNames.AdministrationService), ClientUserSessionData), proxy =>
                    {
                        try
                        {
                            ...
                            tempUser.gcsUserOldPasswords.Clear();
        ...
        public IEnumerable<PasswordValidationResult> ValidatePassword(gcsUser user)
        {
            InitializeErrorsCollection();
            IEnumerable<PasswordValidationResult> validationResult = new List<PasswordValidationResult>();
            WithClient(
                _ServiceFactory.CreateClient<IAdministrationService>(Binding,
                    GetServiceAddress(ServiceNames.AdministrationService), ClientUserSessionData), proxy =>
                    {
                        try
                        {
                            ...
                            validationResult = proxy.ValidatePassword(tempUser);
                        }
                        catch (FaultException<ExceptionDetail> ex)
                        {
                            AddError(new CustomError(ex.Detail));
        ...
        public async Task<IEnumerable<PasswordValidationResult>> ValidatePasswordAsync(gcsUser user)
        {
            InitializeErrorsCollection();
            InitializeErrorsCollection();
            IEnumerable<PasswordValidationResult> validationResult = new List<PasswordValidationResult>();
            ...
            try
            {
                return await WithClientFuncAsync(
                        ...
                        GetServiceAddress(ServiceNames.AdministrationService), ClientUserSessionData), async proxy =>
                    {
                        ...
                        tempUser.UserPassword = Crypto.EncryptString(tempUser.UserPassword,
                            tempUser.UserId.ToString()); //tempUser.LoginName);
                        validationResult = await proxy.ValidatePasswordAsync(tempUser);
