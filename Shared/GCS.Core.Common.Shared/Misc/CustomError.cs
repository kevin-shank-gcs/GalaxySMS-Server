    ...
    public class CustomError
    {
        ...
        public CustomError(ExceptionDetailEx exceptionDetailEx)
        {
            ...
            if (exceptionDetailEx.InnerException != null)
            {
                ...
                ErrorMessage = $"{ex.Message} ({ex.Type})";
            }
            else
            {
                if (exceptionDetailEx.Type == typeof(GCS.Core.Common.Exceptions.DataValidationException).ToString())
                {
                    var kvp = ExceptionDetailEx.ValidationErrors.FirstOrDefault();
                    if (!string.IsNullOrEmpty(kvp.Key))
                    {
                        ErrorMessage = $"{kvp.Key}.{kvp.Value.FirstOrDefault()}";
                    }
                }
                else
                    ErrorMessage = $"{exceptionDetailEx.Message} ({exceptionDetailEx.Type})";
            }
        }
        ...
        public CustomError(ExceptionDetail exceptionDetail)
        {
            ...
            {
                //                ErrorMessage = exceptionDetail.Message;
                ErrorMessage = $"{exceptionDetail.Message} ({exceptionDetail.Type})";
            }
        }
